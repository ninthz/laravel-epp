...
class XmlUtility {
    ...
    private function removeNamespace(Array $array): Array
    {
        $result = [];
        foreach ($array as $key => $value) {
            $keyArr = explode(':', $key);
            $newKey = $keyArr[count($keyArr) - 1];

            if (is_array($value))
                $result[$newKey] = $this->removeNamespace($value);
            else
                $result[$newKey] = $value;
        }
    ...
    public function parseXmlResponse(String $xml, String $resKey = 'resData', String $resStatusKey = null, bool $namespace = false): Array
    {
        ...
        $responseArr = $responseArr['epp']['response'];

        $result = [];

        if ($resStatusKey)
        {
            $status = $responseArr[$resStatusKey] ?? null;
